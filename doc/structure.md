...
### 4.1 播放器模式 (Player Mode) - 4K 横屏版

虽然播放器模式通常是竖屏的，但在 4K 横屏下，我们可以将其重新布局为左右分栏结构：左侧为专辑封面，右侧为歌词和控制栏。

*   **分辨率**: 3840 x 2160
*   **布局思路**:
    *   **专辑封面**: 放置在左侧，垂直居中，放大以占据左半部分视觉重心。
    *   **歌词**: 放置在右侧上方，占据较大空间。
    *   **歌曲信息 & 进度条 & 控制**: 放置在右侧下方。

```javascript
// Player Mode (3840x2160) Configuration
{
    width: 3840,
    height: 2160,
    positions: {
        'album-cover-container': { x: -800, y: 0, scale: 1.2, width: '' }, // 左移并放大
        'lyrics-container': { x: 800, y: -200, scale: 1.3, width: '' },    // 右移并上移
        'track-info': { x: 800, y: 600, scale: 1.1, width: '' },           // 右下
        'progress-section': { x: 800, y: 750, scale: 1.1, width: '' },     // 右下
        'control-buttons': { x: 800, y: 900, scale: 1.1, width: '' },      // 右下
        'watermark': { x: 1600, y: 1000, scale: 1.5, width: '' }           // 右下角
    }
}
```

### 4.2 评价模式 (Review Mode) - 4K 横屏版

评价模式在横屏下可以更好地展示长篇文字，同时保持封面的视觉冲击力。

*   **分辨率**: 3840 x 2160
*   **布局思路**:
    *   **专辑封面**: 较大放置在左上角或左侧。
    *   **标题/歌手**: 紧随封面之下或之上。
    *   **歌词**：占据屏幕右侧上方的大部分区域，5行居中显示，做出渐隐效果。
    *   **水印**: 放置在右下角，保持低调但可见。
    *   **评价正文**: 占据屏幕右侧下方的大部分区域，形成类似杂志排版的阅读体验。